...
SELECT customer_id, RIGHT(customer_id, 5) AS cust_no
FROM customers;
...
-- 2.7: Retrieve a list of the customer number of all customers
...
SELECT first_name, last_name, gender, REPLACE(gender, 'M','Male')
FROM employees;

-- 4.2: Change F to Female in the gender column of the employees table
SELECT first_name, last_name, gender, REPLACE(gender, 'F','Female')
FROM employees;
...
SELECT customer_name, city ||', '||state||', '||country AS full_address
FROM customers;
...
SELECT order_id, STRING_AGG(product_id, ', ') AS products
FROM sales
GROUP BY order_id
ORDER BY order_id;
...
COALESCE(country, city, state, 'No Address') AS cust_addrs
FROM customers;
